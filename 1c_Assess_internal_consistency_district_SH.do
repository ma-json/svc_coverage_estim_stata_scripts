...
* ======================================================================================================================
*   
*   Filename:		1c_Assess_internal_consistency_district
*   Author:			M Johnson
...
* 					Part 2: Identify reporting value outliers over time (internal consistency)

* 					Part 4: Adjust reporting value outliers
* 					Part 5: Correct missing values
...
*-----------------------------------------------------------------------------------------------------------------------
* import data files
use "1b_i_DHIMS2_master_completeness_adjusted_dataset.dta", clear
...
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits tot_stillb stillb_f ///
	...
	bysort orgunitlevel3: egen `var'_mad = mad(`var') if year < ${lastyear} // most recent year is excluded from the assessment of the historical trend so that the earlier years are treated as a 'look-back' period
	bysort orgunitlevel3: egen max_`var'_mad = max(`var'_mad)
	replace `var'_mad = max_`var'_mad if `var'_mad == .
	replace `var'_mad = round(`var'_mad, 1)
	
	bysort orgunitlevel3: egen `var'_med = median(`var') if year < ${lastyear} // most recent year is excluded from the assessment of the historical trend so that the earlier years are treated as a 'look-back' period
	bysort orgunitlevel3: egen max_`var'_med = max(`var'_med)
	replace `var'_med = max_`var'_med if `var'_med == .
	replace `var'_med = round(`var'_med, 1)
	
	bysort orgunitlevel3: gen `var'_outlb2std = round(`var'_med - 1.4826 * 2 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outub2std = round(`var'_med + 1.4826 * 2 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outlb3std = round(`var'_med - 1.4826 * 3 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outub3std = round(`var'_med + 1.4826 * 3 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outlb5std = round(`var'_med - 1.4826 * 5 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outub5std = round(`var'_med + 1.4826 * 5 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outlb8std = round(`var'_med - 1.4826 * 8 * `var'_mad, 1)
	bysort orgunitlevel3: gen `var'_outub8std = round(`var'_med + 1.4826 * 8 * `var'_mad, 1)
	...
	bysort orgunitlevel3: egen `var'_tt_d_outlier5std = total(cond(`var'_outlier5std, 1, 0))
	lab var `var'_tt_d_outlier5std "Total number of `var' outliers by district for 5 STD from the median"
	
	bysort orgunitlevel3 year: egen `var'_tt_dy_outlier5std = total(cond(`var'_outlier5std, 1, 0))
	...
	bysort orgunitlevel3 year: gen `var'_count_dy = _N
...
save "1c_i_Internal_consistency_outliers_over_time_district.dta", replace		
...
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits tot_stillb stillb_f ///
	...
	bysort orgunitlevel3 year: egen med_`var' = median(`var') if `var'_outlier5std != 1
	replace med_`var' = round(med_`var', 1)
	
	bysort orgunitlevel3 year: egen max_`var' = max(med_`var')
	replace `var' = max_`var' if `var'_outlier5std == 1
	drop med_`var' max_`var'
}

// generate lists of districts with adjusted outliers
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits tot_stillb stillb_f ///
	stillb_m u5_death mat_death {
	list orgunitlevel3 year raw_`var' `var' if `var'_outlier5std == 1
}
...
* save dataset in stata format
save "1c_ii_DHIMS2_master_completeness_outliers_adjusted_dataset.dta", replace
...
*-----------------------------------------------------------------------------------------------------------------------
* import data files
use "1c_ii_DHIMS2_master_completeness_outliers_adjusted_dataset.dta", clear
...
// tally missing data for all health indicator/district/month combinations
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits total_stillbirth stillbirth_f ///
	stillbirth_m under5_deaths maternal_deaths {
	count if `var' == .
	tab mis_`var'
	list orgunitlevel3 year month `var' mis_`var' if mis_`var' == 1, abbrev(20) noobs
}

// correct missing values
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits total_stillbirth stillbirth_f ///
	stillbirth_m under5_deaths maternal_deaths {
	bysort orgunitlevel3 year: egen total_mis_`var' = total(mis_`var')
	bysort orgunitlevel3 year: egen count_mis_`var' = count(mis_`var')
	...
	bysort orgunitlevel3 year: egen median_`var' = median(`var') if mis_`var' != 1
	replace median_`var' = round(median_`var', 1)
	
	bysort orgunitlevel3 year: egen max_median_`var' = max(median_`var')
...
compress
save "1c_iii_DHIMS2_master_adjusted_dataset.dta", replace
