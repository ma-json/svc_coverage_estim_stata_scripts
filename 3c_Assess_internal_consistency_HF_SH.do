...
* ======================================================================================================================
*   
*   Filename:		3c_Assess_internal_consistency_HF
*   Author:			M Johnson
...
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits tot_stillb stillb_f ///
	...
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: egen `var'_mad = mad(`var') if year < ${lastyear} 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: egen max_`var'_mad = max(`var'_mad)
	replace `var'_mad = max_`var'_mad if `var'_mad == .
	replace `var'_mad = round(`var'_mad, 1)
	
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: egen `var'_med = median(`var') if year < ${lastyear} 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: egen max_`var'_med = max(`var'_med)
	replace `var'_med = max_`var'_med if `var'_med == .
	replace `var'_med = round(`var'_med, 1)
	
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outlb2std = round(`var'_med - 1.4826 * 2 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outub2std = round(`var'_med + 1.4826 * 2 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outlb3std = round(`var'_med - 1.4826 * 3 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outub3std = round(`var'_med + 1.4826 * 3 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outlb5std = round(`var'_med - 1.4826 * 5 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outub5std = round(`var'_med + 1.4826 * 5 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outlb8std = round(`var'_med - 1.4826 * 8 * `var'_mad, 1) 
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: gen `var'_outub8std = round(`var'_med + 1.4826 * 8 * `var'_mad, 1) 
	...
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid: egen `var'_tt_d_outlier5std = total(cond(`var'_outlier5std, 1, 0))
	lab var `var'_tt_d_outlier5std "Total number of `var' outliers by health facility for 5 STD from the median"
	
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen `var'_tt_dy_outlier5std = total(cond(`var'_outlier5std, 1, 0))
	lab var `var'_tt_dy_outlier5std "Total number of `var' outliers by health facility & year for 5 STD from the median"
	
	bysort year: egen `var'_tt_y_outlier5std = total(cond(`var'_outlier5std, 1, 0))
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: gen `var'_count_dy = _N
...
save "3c_i_Internal_consistency_outliers_over_time_hf.dta", replace		
...
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits tot_stillb stillb_f ///
	...
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen med_`var' = median(`var') if `var'_outlier5std != 1 
	replace med_`var' = round(med_`var', 1)
	
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen max_`var' = max(med_`var')
...
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits tot_stillb stillb_f ///
	stillb_m u5_death mat_death {
	list orgunitlevel3 subdistrict_group organisationunitname organisationunitid year raw_`var' `var' if `var'_outlier5std == 1
}
...
* save dataset in stata format
save "3c_ii_DHIMS2_master_completeness_outliers_adjusted_dataset.dta", replace
...
*-----------------------------------------------------------------------------------------------------------------------
* import data files
use "3c_ii_DHIMS2_master_completeness_outliers_adjusted_dataset.dta", clear
...
// tally missing data for all health indicator/facility/month combinations
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits total_stillbirth stillbirth_f ///
	stillbirth_m under5_deaths maternal_deaths {
	count if `var' == .
	tab mis_`var'
	list orgunitlevel3 subdistrict_group organisationunitname organisationunitid year month `var' mis_`var' if mis_`var' == 1, abbrev(20) noobs
}

// correct missing values
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits total_stillbirth stillbirth_f ///
	stillbirth_m under5_deaths maternal_deaths {
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen total_mis_`var' = total(mis_`var')
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen count_mis_`var' = count(mis_`var')
	...
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen median_`var' = median(`var') if mis_`var' != 1 
	replace median_`var' = round(median_`var', 1)
	
	bysort orgunitlevel3 subdistrict_group organisationunitname organisationunitid year: egen max_median_`var' = max(median_`var')
...
compress
save "3c_iii_DHIMS2_master_adjusted_dataset.dta", replace
