...
* ======================================================================================================================
*   
*   Filename:		1a_Prepare_data_district
*   Author:			M Johnson
...
* 					Part 3: Create data files for completeness of reporting by region/year
* 					Part 4: Create data files for completeness of reporting by district/year
* 					Part 5: Create data files to identify low reporting by district/year
* 					Part 6: Identify reporting rate outliers by district/year
...
import excel "0c_iii_DHIMS2_template_svc_data_1_distr.xlsx", sheet("Sheet 1") firstrow case(lower) clear
save "0c_iii_DHIMS2_template_svc_data_1_distr.dta", replace

import excel "0c_iv_DHIMS2_template_svc_data_2_distr.xlsx", sheet("Sheet 1") firstrow case(lower) clear
save "0c_iv_DHIMS2_template_svc_data_2_distr.dta", replace

import excel "0c_v_DHIMS2_template_svc_data_3_distr.xlsx", sheet("Sheet 1") firstrow case(lower) clear
save "0c_v_DHIMS2_template_svc_data_3_distr.dta", replace

// merge files
use "0c_iii_DHIMS2_template_svc_data_1_distr.dta", clear

merge 1:1 orgunitlevel3 year month using "0c_iv_DHIMS2_template_svc_data_2_distr.dta"
list orgunitlevel3 year month _merge if _merge != 3	// to check discrepancy of key variables for merging
drop _merge

merge 1:1 orgunitlevel3 year month using "0c_v_DHIMS2_template_svc_data_3_distr.dta"
list orgunitlevel3 year month _merge if _merge != 3	// to check discrepancy of key variables for merging
drop _merge
...
order orgunitlevel3 year _month // order variables in the dataset
sort orgunitlevel3 year _month // sort the dataset by listed variables

save "1a_i_DHIMS2_template_svc_data_all_distr.dta", replace


* import reporting completeness file
import excel "0c_xii_DHIMS2_template_rep_compl_distr.xlsx", sheet("Sheet 1") firstrow case(lower) clear
...
order orgunitlevel3 year _month
sort orgunitlevel3 year _month

save "1a_ii_DHIMS2_template_rep_compl_all_distr.dta", replace
...
use "1a_i_DHIMS2_template_svc_data_all_distr.dta", clear

merge 1:1 orgunitlevel3 year month using "1a_ii_DHIMS2_template_rep_compl_all_distr.dta"
list orgunitlevel3 year month _merge if _merge != 3	//*Check discrepancy of key variables for merging
drop _merge

merge m:1 orgunitlevel3 year using "1a_iii_DHIMS2_template_pop_data.dta"
list orgunitlevel3 year month _merge if _merge != 3	//*Check discrepancy of key variables for merging
drop _merge

merge m:1 orgunitlevel3 using "1a_iv_DHIMS2_template_admin_data.dta"
list orgunitlevel3 year month _merge if _merge != 3	//*Check discrepancy of key variables for merging
drop _merge

order orgunitlevel3 year _month month
sort orgunitlevel3 year _month
...
save "1a_v_DHIMS2_template_master_non_adjusted_dataset.dta", replace
...
*-----------------------------------------------------------------------------------------------------------------------
* import data files
use "1a_v_DHIMS2_template_master_non_adjusted_dataset.dta", clear

keep orgunitlevel3 year month anc_rr idelv_rr pnc_rr vacc_rr opd_rr diar_rr ipd_rr fp_rr
...
save "1a_vi_DHIMS2_completeness_by_region_year.dta", replace
...
foreach var of varlist anc_rr idelv_rr pnc_rr vacc_rr opd_rr diar_rr ipd_rr fp_rr {  
	preserve
	collapse (mean) `var', by(orgunitlevel3 year)
...
local reshapeddistrict: dir . files "_completeness_district*.dta"
	...
	merge 1:1 orgunitlevel3 year using "`file'"
drop _merge
}

* save dataset in stata format
save "1a_vii_DHIMS2_completeness_by_district_year.dta", replace
...
reshape wide anc_rr fp_rr idelv_rr pnc_rr ipd_rr opd_rr diar_rr vacc_rr, i(orgunitlevel3) j(year)

* flag districts with reporting completeness below the 'low' DQ threshold of 90% (i.e. % of expected health facilities with a reported value) by service type in 2017
gen lowc_17_anc = 0
...
lab var lowc_17_opd "District with (all ages) diarrhoea service events reporting below ${threshold_low_rr}% in 2017"
...
* flag districts with reporting completeness below the 'low' DQ threshold of 90% (i.e. % of expected health facilities with a reported value) by service type in 2018
gen lowc_18_anc = 0
...
lab var lowc_18_diar "District with (all ages) diarrhoea service events reporting below ${threshold_low_rr}% in 2018"
...
* flag districts with reporting completeness below the 'low' DQ threshold of 90% (i.e. % of expected health facilities with a reported value) by service type in 2019
gen lowc_19_anc = 0
...
lab var lowc_19_diar "District with (all ages) diarrhoea service events reporting below ${threshold_low_rr}% in 2019"
...
* flag districts with reporting completeness below the 'low' DQ threshold of 90% (i.e. % of expected health facilities with a reported value) by service type in 2020
gen lowc_20_anc = 0
...
lab var lowc_20_diar "District with (all ages) diarrhoea service events reporting below ${threshold_low_rr}% in 2020"
...
save "1a_vii_DHIMS2_LOW_completeness_by_district_year.dta", replace
...
* save dataset in stata format
save "1a_viii_DHIMS2_LOW_completeness_by_year.dta", replace


*-----------------------------------------------------------------------------------------------------------------------
* Part 6: Identify reported outliers by district/year ####
*-----------------------------------------------------------------------------------------------------------------------
* import data files
use "1a_v_DHIMS2_template_master_non_adjusted_dataset.dta", clear
...
foreach var of varlist anc1 anc4 ipt2 idelv csection pnc48h bcg penta1 penta2 penta3 measles1 diar_total diar_under1 diar_under5 opd_total opd_under5 ipd_total ipd_under5 fp_total fp_new fp_revisits total_stillbirth stillbirth_f ///
	...
	bysort orgunitlevel3: egen maxlowc_`var' = max(lowc_`var')	
	replace maxlowc_`var' = 1 if maxlowc_`var' >= 1
	
	bysort orgunitlevel3: egen rr_`var'_med = median(rr_`var') if rr_`var' >= 75
	bysort orgunitlevel3: egen max_rr_`var'_med = max(rr_`var'_med)
